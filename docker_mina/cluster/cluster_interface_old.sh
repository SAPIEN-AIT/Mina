...
submit_job() {
    ...
    ssh $CLUSTER_LOGIN "cd $CLUSTER_ISAACLAB_DIR && bash $CLUSTER_ISAACLAB_DIR/docker/cluster/$job_script_file \"$CLUSTER_ISAACLAB_DIR\" \"$profile\" ${@}"
}
...
case $command in
    push)
        ...
        rm -rf /$SCRIPT_DIR/exports/$profile*
        ...
        APPTAINER_NOHTTPS=1 apptainer build --sandbox --fakeroot $profile.sif docker-daemon://$profile:latest
        # tar image (faster to send single file as opposed to directory with many files)
        tar -cvf /$SCRIPT_DIR/exports/$profile.tar $profile.sif
        # make sure target directory exists
        ssh $CLUSTER_LOGIN "mkdir -p $CLUSTER_SIF_PATH"
        # send image to cluster
        scp $SCRIPT_DIR/exports/$profile.tar $CLUSTER_LOGIN:$CLUSTER_SIF_PATH/$profile.tar
        ;;
    job)
        if [ $# -ge 1 ]; then
            passed_profile=$1
